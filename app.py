...
def get_current_user():
    ...
    # Helper function to check if the user is an admin, advisor, or IT staff
def is_admin(user):
...
def book_appointment():
    ...
    # If the user is an advisor, they will only see their own students
    if user['isadmin'] == 2:  # Advisor role
        cursor.execute('SELECT advisor_id, name FROM Advisors WHERE advisor_id = ?', (user['user_id'],))
        advisors = cursor.fetchall()
        
        cursor.execute('SELECT user_id, username FROM Users WHERE advisor_id = ?', (user['user_id'],))  # Show only students assigned to this advisor
        students = cursor.fetchall()
    
    # If the user is an admin or IT staff, show all advisors and students
    elif user['isadmin'] == 1 or user['isadmin'] == 3:  # Admin or IT staff
        cursor.execute('SELECT advisor_id, name FROM Advisors')
        advisors = cursor.fetchall()
        
        cursor.execute('SELECT user_id, username FROM Users')  # Admin/IT staff see all students
        students = cursor.fetchall()
    
    # If the user is a student, they should only see their own advisor
    else:  # Student role (assuming 'isadmin' value for students is 0)
        cursor.execute('SELECT advisor_id FROM Users WHERE user_id = ?', (user['user_id'],))
        assigned_advisor_id = cursor.fetchone()[0]
        
        cursor.execute('SELECT advisor_id, name FROM Advisors WHERE advisor_id = ?', (assigned_advisor_id,))
        advisors = cursor.fetchall()  # Only the assigned advisor
        students = []  # Students do not need a list of other students
    
    conn.close()

    return render_template('book_appointment.html', username=user['username'], advisors=advisors, students=students)
...
def admin_dashboard():
    user = get_current_user()
    if not user or user['isadmin'] != 1:  # Only admins can access this
        return redirect(url_for('login'))
...
def users():
    ...
    # Restrict access to admin and IT staff for managing users
    if user['isadmin'] not in [1, 3]:  # Only admin and IT staff can access this
        return redirect(url_for('dashboard'))

    conn = connect_db()
    cursor = conn.cursor()

    # For Advisors, restrict access to their assigned students
    if user['isadmin'] == 2:  # Advisor
        cursor.execute('SELECT user_id, username FROM Users WHERE advisor_id = ?', (user['user_id'],))
    else:
        cursor.execute('SELECT user_id, username FROM Users')  # Admin/IT staff see all users
...
def appointments():
    ...
    if user['isadmin'] == 2:  # Advisor: Only view assigned students
        cursor.execute(''' 
            SELECT a.appointment_id, u.username AS student, adv.name AS advisor, 
                   ts.available_date AS appointment_date, ts.time_slot
            FROM Appointments a
        ...
        ''', (user['user_id'],))  # Only show appointments for assigned advisor
    else:
        cursor.execute(''' 
            SELECT a.appointment_id, u.username AS student, adv.name AS advisor, 
                   ts.available_date AS appointment_date, ts.time_slot
    ...
    conn.close()
    return render_template('partials/appointments.html', appointments=appointments)
...
def addappointmentslots():
    ...
    # Restrict access: Only admin or IT staff can add appointment slots
    if user['isadmin'] not in [1, 3]:
        return redirect(url_for('dashboard'))
...
def addcourse():
    ...
    # Only allow IT Staff or Admin to add courses
    if user['isadmin'] not in [1, 3]:  # IT staff or Admin
        return redirect(url_for('dashboard'))

    # Handling POST request to add course
    if request.method == 'POST':
        ...
        return render_template('partials/addcourse.html', success='Course added successfully!')
